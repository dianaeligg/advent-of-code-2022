...
instructionsArr.forEach(({ howMany, from, to }) => {
  ...
  for (i = 0; i < howMany; i++) {
    const mouthful = stacks[from].length - howMany + i;
    console.log({ mouthful, letter: stacks[from][mouthful] });
    stacks[to].push(stacks[from][mouthful]);
  }
  for (i = 0; i < howMany; i++) {
    stacks[from].pop();
  }
  console.log({ stacks: Object.values(stacks) });
});
